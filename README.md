...
- **不分析投影片，只分析音訊**：不擷取、不匯入也不理解任何投影片，摘要只根據
  逐字稿產生。
...
介面會依來源自動防呆：輸入 YouTube 網址會清除已上傳檔案、自動切換至「不分析
投影片，只分析音訊」，並以鎖定標示阻擋「自動從影片偵測並擷取」；上傳檔案則會
清除 YouTube 網址。音檔同樣會鎖定自動擷取模式，只有上傳影片檔時才會解除鎖定。
URL 與音檔仍可改選「使用之前已擷取的投影片」。
